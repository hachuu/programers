...
function solution(n, words) {
    ...
    sorted = sorted.filter((x,i)=>i>1&&x === sorted[i-1]);
    if (sorted.length > 0) {
        let index = 0;
        let cnt = 0;
        origin.map((x,i) => {
        ...
        if (ordering === 0) {
            ...
            return [ordering,  gameNumber];
        }
    } else {
        const wrongArray = JSON.parse(JSON.stringify(origin));
        const wrong = origin.filter((x, i)=>i>0 && x.substr(0, 1) !== origin[i-1].substr(-1, 1));
