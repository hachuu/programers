/*
	문제: 2019 카카오 개발자 겨울 인턴십 크레인 인형뽑기 게임
			...
			if (boards[i][jIdx] !== 0) {
				if (j===0) {
                    ...
                    if (boards[i][jIdx] !== 0) {
                        ...
                        boards[i][jIdx] = 0;
                        console.log('쌓인 인형 dollTopArray : ', dollTopArray);
                        console.log('바뀐 원래 인형 :', boards);
                        continue loop1;
                    }
				} else {
                    console.log('i :', i);
                    console.log('jIdx :', jIdx);
                    console.log('쌓여야할 인형  : ', boards[i][jIdx]);
                    console.log('쌓여잇는 마지막 인형 :' ,dollTopArray[dollTopArray.length-1]);
					if (dollTopArray[dollTopArray.length-1] === boards[i][jIdx]) {
						console.log('현재 쌓여있는 인형 dollTopArray : ', dollTopArray);
						dollTopArray.pop();
						console.log('사라지고 남은 dollTopArray : ', dollTopArray);
						disapearCnt = disapearCnt+2;
                        console.log('사라진 인형cnt : ', disapearCnt);
                        boards[i][jIdx] = 0;
                        console.log('바뀐 원래 인형 :', boards);
						continue loop1;
					} else {
                        dollTopArray.push(boards[i][jIdx]);
                        boards[i][jIdx] = 0;
                        console.log('전거랑 달라서 쌓인 인형 : ', dollTopArray);
                        console.log('바뀐 원래 인형 :', boards);
                        continue loop1;
                    }
...
// 다른 사람 풀이 => 난 아직 reduce의 활용법을 잘 모르는 거 같다.. reduce 공부가 필요하다.
